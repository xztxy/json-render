...
export const schema = defineSchema(
  ...
  }),
  {
    defaultRules: [
      ...
      'When building repeating content backed by a state array (e.g. todos, posts, cart items), use the "repeat" field on a container element from the AVAILABLE COMPONENTS list. Example: { "type": "<ContainerComponent>", "props": { "gap": 8 }, "repeat": { "path": "/todos", "key": "id" }, "children": ["todo-item"] }. Inside repeated children, use { "$item": "/field" } to read a field from the current item, and { "$index": true } for the current array index. For two-way binding use statePath:"$item/completed". Do NOT hardcode individual elements for each array item.',

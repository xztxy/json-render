...
export const schema = defineSchema(
  ...
  }),
  {
    defaultRules: [
      ...
      'When building repeating content backed by a state array (e.g. posts, products, items), use the "repeat" field on a container element. Example: { "type": "<ContainerComponent>", "props": {}, "repeat": { "path": "/posts", "key": "id" }, "children": ["post-card"] }. Replace <ContainerComponent> with an appropriate component from the AVAILABLE COMPONENTS list. Inside repeated children, use { "$item": "/field" } to read a field from the current item, and { "$index": true } for the current array index. For two-way binding use statePath:"$item/completed". Do NOT hardcode individual elements for each array item.',

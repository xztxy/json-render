...
For two-way binding (form inputs), use `{ "$bind": "/path" }` on the natural value prop. The renderer provides a `bindings` map with the state path for each bound prop. Use `useBoundProp` to get `[value, setValue]`:

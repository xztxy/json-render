...
### Changed: `onStateChange` no longer fires for no-op updates

The `onStateChange` callback is now only called when a `set()` or `update()` call actually changes the state. Previously it fired unconditionally, even when the new value was identical to the current value. If your code relied on the callback firing for every `set()` call regardless of whether the value changed, you may need to adjust.

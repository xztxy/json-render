...
The `useFieldValidation` and `useStateStore` hooks wire validation into your registry components:

```tsx
import { useFieldValidation, useStateStore } from '@json-render/react';
...
function TextField({ props }) {
  ...
  const value = state[props.statePath] as string;
